     ...
     
{
	...
	{                         /*create multi-dimensional array*/
    		...
    		else
		{
			...
			/*malloc array*/ 
			for (i=0;i<n;i++) count[i] = (int *)malloc(m*sizeof(int));     
			...
			{                        /*initialize array*/
   			...
   			count[ibug][jbug]=1;
     			...
     			while ((counter != n*m)&&(iteration < 50000))
			{    /*begin while*/
				move = random () % 8;
				imaybe = ibug + imove[move];              /*choose random move*/
				jmaybe = jbug + jmove[move];
				if ((imaybe >= 0) && (imaybe < n) && (jmaybe >= 0) && (jmaybe < m))
				{      /*check validity*/
	  
		  			ibug = imaybe;
	  				jbug = jmaybe;                  /*if valid do these things*/
	  				if (count[ibug][jbug] == 0) counter = counter + 1;               
	  				count[ibug][jbug]++;
        			}

...
menu() {   
  ...
  while (foo != 'q'){
	...
	if ((max_row1 > 0) && (max_row1 <= 20)){max_col1 = max_col1;}
	else {exit(-1);}
		
	printf("MAX_COL>");
	scanf("%d",&max_col1);
	
	if ((max_col1 > 0) && (max_col1 <= 20)){max_col1 = max_col1;}
	else {exit(-1);}
	...
	for(i=1;i<=number_of_nodes;i++){                          /*MATRIX 1*/
	  ...
	  if ((row1 > 0) && (row1 <= max_row1)){row1 = row1;}
	  else {exit(-1);}
	  
	  printf("COL>");
	  scanf("%d",&col1);    
	  
	  if ((col1 > 0) && (col1 <= max_col1)){col1 = col1;}
	  else {exit(-1);}
	...
	scanf("%d",&max_row1);
	rewind(stdin);
	if ((max_row1 > 0) && (max_row1 <= 20)){max_row1 = max_row1;}
	else {   }

...
void init_buffer_index(int *s_index, int *d_index, 
		       int *f_index, int *c_index);
void write_buff(char buff[BUF_SIZE],int fd);   
void get_write_fd(int *a, int *b, int *c, int *d);
int get_read_fd (char *file);
int find_char_bjubb (char find_me, char ***buffer, 
		     int position,
		     int *which);
int init_start_values(char ***buffer, 
		      int *data_index, 
		      int *format_index,
		      int *format_count,
                      int *which,
	    	      int *data_which);
void do_string(int length, char ***buffer, 
	       int fd, char buff[BUF_SIZE],
	       int *data_index, int *s_index,
	       int *which,
	       int *data_which);
void do_int(int length, char ***buffer, 
	    int fd, char buff[BUF_SIZE],
	    int *data_index, int *d_index,
	    int *which,
	    int *data_which);
void do_char(char ***buffer, int fd,
	     char buff[BUF_SIZE],
	     int *data_index, int *c_index, int *data_which);
void do_float(int length1, int length2, char ***buffer,
	      int fd, char buff[BUF_SIZE], 
	      int *data_index, int *f_index,
	      int *which,
	      int *data_which);
void check_buff(int *output_buffer_index,
...
main (int argc, char *argv[]) {  
  ...
  init_start_values(BUFFER_ADDRESS, 
		    DATA_INDEX_ADDRESS,
		    &format_index,
		    &format_count, 
		    &WHICH,
...
/*******************************************************************/
  for (j = 0; j < (format_count+1) ; j++ ){ 
	...
	do_string (s_length, BUFFER_ADDRESS, s,
		   s_buffer, DATA_INDEX_ADDRESS, &s_index,
		   &WHICH,
		   &data_WHICH);
	j++; 
	format_index += 2;
	...
	do_int (d_length, BUFFER_ADDRESS, d,
	d_buffer, DATA_INDEX_ADDRESS, &d_index,
	&WHICH,&data_WHICH); 
	j++; 
	format_index +=2;
	break; 
	...
	do_float(f_length1, f_length2, BUFFER_ADDRESS, f,
		 f_buffer, DATA_INDEX_ADDRESS, &f_index,
		 &WHICH,
		 &data_WHICH);
		 j += 3;
	format_index += 4;
	...
	if (find_char_bjubb ('(',BUFFER_ADDRESS,
			     data_index,
			     &WHICH)==0){
	  break;
	}
	else {
	  data_index = find_char_bjubb(')',
				       BUFFER_ADDRESS,
				       data_index,
				       &data_WHICH)+1;
	  j = 0;
...
/*******************************************************************/
void do_string(int length, char ***in,
	       int fd, char s_buffer[BUF_SIZE],
	       int *data_index, int *s_buffer_index,
...
/*******************************************************************/
void do_int(int length, char ***in,
	    int fd, char d_buffer[BUF_SIZE],
	    int *data_index, int *d_buffer_index,
	    int *which,
	    int *data_which) {
  int i;
  int j;
...
/*******************************************************************/
void do_float(int length1, int length2, char ***in,
	      int fd, char f_buffer[BUF_SIZE],
	      int *data_index, int *f_buffer_index,
	      int *which,
	      int *data_which){
  int i , j, stop1, stop2;
  *f_buffer_index;
  ...
  for (i = (*data_index), j = (*f_buffer_index); 
       i < stop1; i++,j++) { 
    check_buff(f_buffer_index, f_buffer, &fd);
...
void check_buff(int *output_buffer_index,
		char output_buff[BUF_SIZE], 
		int *fd) 
{   /* what the hell am i doing in here...oh yeah...
...
/*******************************************************************/
void init_buffer_index(int *sindex, int *dindex, int *findex,
			int *cindex){
  *sindex = 0;
  *dindex = 0;
  *findex = 0;
		    ...
		    int *data_which)
{
  ...
  while (i < BUF_SIZE)  
    {
      if ((*buffer)[*data_which][i] == find_me) return (i);
      else i++;
    }
  return(0);
}

...
#define send_token(s, t)                                                       \
  {                                                                            \
    tToken _t = t;                                                             \
    write(s, &_t, TOKEN_SIZE);                                                 \
  }
...
main(int argc, char *argv[]) {
  ...
  if (argc < 5) {
    printf("usage: %s <my #> <max # of processes> <port adjuster> <process "
           "host list>\n",
           argv[0]);
    ...
    printf("				        be added to the PORT_BASE of "
           "\n");
    printf("				        of 5200. YOU MUST ENTER A "
           "VALUE\n");
  ...
  port_adjuster =
      atoi(argv[3]); /* this is for my  'port adjuster' that allows you to
                        change the port number 'on the fly'. */
...
run(tSocket others[], int my_number, int next, int max_processes) {
  ...
  while (TRUE) {
    ...
    default:
      printf("Hey where the hell is the token?\n");
      printf("start election.\n");
      /*	election=TRUE; */
      ...
      /*	else if(token.as_char[1]>my_number)
              ...
              {
                      printf("im sending %d an election token with
         my_number=%d\n",next,my_number);
                      send_token(others[token.as_char[1]],create_new_token('e',my_number));
              }
              break;
...
setup(tSocket others[], int my_number, int max_processes, char *hosts[],
  ...
  for (i = 0; i < max_processes; i++) {
    if (i == my_number) {
      ...
      if (others[i] < 0) {
        char msg[50];
        sprintf(msg, "ABORT! can't open socket to %d returned %d", i,
                others[i]);
        perror(msg);
        exit(-1);
      }
...
tToken receive_any(
    tSocket others[], int my_number,
    int max_processes) { /* check to see if anyone is sending me a token */
  tToken token;
  long start, stop;
  int i;
  ...
  /*	printf("im here\n"); */
  for (start = time((time_t *)0), stop = time((time_t *)0); stop - start < 3;
       stop = time((time_t *)0)) {

...
{  
	...
	for (i=st->current_scope;i>=0;i--) 
        if (ht_lookup (st->scopes[i], symbol)) return (i);
	return (-1);
...
int st_open_scope(tSymbolTable st)
{
	...
	st->current_scope++;
	st->scopes[st->current_scope]=ht_initialize(SCOPE_HT_SIZE,st->symbol_compare_function,st->symbol_hash_function);
	return(1);
...
{
    if (st->current_scope == -1) return(-1);

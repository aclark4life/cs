...
//
//    1.  Parse the pattern, and layout the balls to provide a list of
//    instructions
//        such as "ball 1 is thrown by hand 0 at t=1.2 seconds and is caught by
//        hand 1 at t=2.5 seconds", etc.  Note that none of the spatial
//        information has been layed out yet.  You can specify different ball
//        path types (e.g. bounce juggling, etc.) just by extending the
//        "BallPath" class.
//
//    2.  Lay out the positions of the hands as a function of time.  After this
//        stage, we can convert the ball-path instructions from step 1 above
//        into actual paths, since we now know where the hands are at the
//        catching and throwing times.  After telling each BallPath object in
//        the list where its starting and ending positions are, the objects in
//        the list are individually responsible for calculating ball positions
//        at all times between these endpoints defined by the hands.  (A
//        BallPathAir object uses a parabolic path in space, for example.)
//
...
abstract class PhysicalPattern {
  ...
  public void setConstants(PhysicalConst c) { con = new PhysicalConst(c); }

  public PhysicalConst getConstants() { return new PhysicalConst(con); }
...
abstract class PhysicalHands {
  ...
  public abstract void layoutHands(BallPathList bpl, double time)
      throws JuggleException;
}
...
class PhysicalGroup {
  ...
  PhysicalGroup(PhysicalPattern pat, PhysicalHands hands, PhysicalJuggler jug,
    ...
    hands.layoutHands(pat.getBallPathList(),
                      (startup_length + loop_period + pat.getMaxThrow() + 2) /
                          con.tps);
...
// The following class is used to control the applet from outside (another
// applet).
...
class JuggleControls {
  ...
  JuggleControls(JuggleControls jc) {
    if (jc.modestring != null)
      this.modestring = jc.modestring;
    if (jc.patstring != null)
      this.patstring = jc.patstring;
    if (jc.ballimage != null)
      this.ballimage = jc.ballimage;
    if (jc.accel >= 0.0)
      this.accel = jc.accel;
    if (jc.tps >= 0.0)
      this.tps = jc.tps;
    if (jc.dwell >= 0.0)
      this.dwell = jc.dwell;
    if (jc.balldiam >= 0.0)
      this.balldiam = jc.balldiam;
    if (jc.bouncefrac >= 0.0)
      this.bouncefrac = jc.bouncefrac;
    if (jc.slowdown >= 0.0)
      this.balldiam = jc.balldiam;
    if (jc.fps >= 0.0)
      this.fps = jc.fps;
    ...
    if (jc.handpath != null)
      this.handpath = jc.handpath;
    if (jc.mat_hr >= 0.0)
      this.mat_hr = jc.mat_hr;
  }
}
          ...
          try {
            for (tempint = 0; tempint < size / 2; tempint++) {
              for (int i = 0; i < 2; i++) {
                ...
                jc.handpath[2 * tempint + i][0] =
                    0.025 * Double.valueOf(st2.nextToken()).doubleValue();
                jc.handpath[2 * tempint + i][1] =
                    0.05 * Double.valueOf(st2.nextToken()).doubleValue();
              }
            }
          } catch (NumberFormatException e) {
            throw new JuggleException();
  ...
  public void run() { /* Called when this object becomes a thread */
    ...
    try {
      ...
      if (pat != null) {
        ...
        r = new Rectangle(jc.border, jc.border, appWidth - 2 * jc.border,
                          appHeight - 2 * jc.border);

        // The following is a hack which adjusts tps to
        // achieve the desired maximum throw height.
        if (jc.mat_hr > 0.0) {
          try {
            jc.tps = Math.sqrt(jc.accel /
                               (8.0 * jc.mat_hr * (double)pat.getMaxThrow())) *
                     ((double)pat.getMaxThrow() - jc.dwell);
          } catch (ArithmeticException e) {
          }
        }

        con = new PhysicalConst(jc.accel, jc.tps, jc.dwell, jc.handscoop,
                                jc.balldiam, jc.bouncefrac);
        ...
        if (jc.ballimage == null)
          jm = new JuggleMovieSimple(group, jc.balldiam, this, r, jc.fps,
                                     jc.slowdown);
        else
          jm = new JuggleMovieSimple(group, jc.ballimage, this, r, jc.fps,
                                     jc.slowdown);
        loaded = true;
      }
      ...
      while (true) {
        ...
        try {
          time_wait =
              jm.get_duration() - System.currentTimeMillis() + time_start;
          if (time_wait > 0)
...
class JuggleMovie {
  ...
  public void add_line(Coordinate c_start, Coordinate c_end)
      throws JuggleException {
    if (frame_status == 0)
  ...
  public void add_ellipse(Coordinate c_start, Coordinate c_end)
      throws JuggleException {
    if (frame_status == 0)
  ...
  public void draw_frame(Graphics g, Component comp) {
    ...
    for (i = 0; i < current_frame.frame_images; i++) {
      ...
      g.drawImage(img, current_frame.images_x[i], current_frame.images_y[i],
                  comp);
    }
  }

  public int get_duration() { return current_frame.frame_duration; }
  ...
  public void coord_to_xy(double zoom, int originx, int originy, int ballwidth,
    ...
    for (i = 0; i < framelist.size(); i++) {
      ...
      for (j = 0; j < fr.frame_ellipses; j++) {
        fr.ellipses_startx[j] =
            originx + (int)(0.5 + zoom * fr.ellipses_scoord[j].x);
        fr.ellipses_starty[j] =
            originy - (int)(0.5 + zoom * fr.ellipses_scoord[j].y);
        fr.ellipses_endx[j] =
            originx + (int)(0.5 + zoom * fr.ellipses_ecoord[j].x);
        fr.ellipses_endy[j] =
            originy - (int)(0.5 + zoom * fr.ellipses_ecoord[j].y);
      }
      for (j = 0; j < fr.frame_images; j++) {
        fr.images_x[j] =
            originx - ballwidth / 2 + (int)(0.5 + zoom * fr.images_coord[j].x);
        fr.images_y[j] =
            originy - ballheight - (int)(0.5 + zoom * fr.images_coord[j].y);
      }
    }
  }
...
class JuggleMovieSimple extends JuggleMovie {
  ...
  JuggleMovieSimple(PhysicalGroup group, double balldiam, Component comp,
                    Rectangle r, double fps, double slowdown)
      throws JuggleException {
    super(group.getBallPathList().getActors() +
              group.getHandPathList().getActors(),
          group.getJuggler().getMaxLines(),
          group.getJuggler().getMaxEllipses());
  ...
  protected void makeMovie(double fps, double slowdown) throws JuggleException {
    ...
    if (ballimage == null) { // need to make our own ball
      frame_width =
          2.0 * Math.max(Math.abs(coordmax.x), Math.abs(coordmin.x)) + balldiam;
...
class BallPathAir extends BallPath {
  ...
  BallPathAir(double accel) { ay = 0.5 * accel; }
...
class BallPathBounce extends BallPath {
  ...
  BallPathBounce(double accel, double bounceplane, double bouncefrac,
                 boolean forced) {
    ay1 = ay2 = 0.5 * accel;
  ...
  protected void fixInternalVariables() {
    ...
    if ((start_coord != null) && (end_coord != null)) {
      ...
      k2 =
          -t2 * (2 * ay2 + bouncefracsqrt * ay1) / (ay2 + bouncefracsqrt * ay1);
      k1 = (ay2 * t2 * t2 + bounceplane - end_coord.y +
            bouncefracsqrt * (bounceplane - cy1)) /
           (ay2 + bouncefracsqrt * ay1);
      k0 = -t2 * bouncefracsqrt * (bounceplane - cy1) /
           (ay2 + bouncefracsqrt * ay1);
  ...
  static protected double findRoot(double k0, double k1, double k2, double t1,
                                   double t2) {
...
// The next class trivially extends the above case to describe the period of
// time when a ball is considered "held", as opposed to that when it is
// "scooped". See BallPathList.findNextThrow() below.
...
class BallPathList {
  ...
  public void setStartPoint(int actor, int hand, double time)
      throws JuggleException {
...
class PhysicalAsyncSiteSwap extends PhysicalPattern {
  ...
  public void layoutPattern(double startup_time, double loop_time)
      throws JuggleException {
      ...
      while (time < total_time) {
        ...
        if (throw_value == 1) {
          ...
          if (mod_value == 'B')
            ...
            bp = new BallPathAir(-con.g);
          bp.setPath((throw_value % 2 == 0) ? throw_hand : (1 - throw_hand),
                     temp);
          bpl.addPath(i, bp);
          time += temp;
          borrow_time = 0.0;
...
class PhysicalHandsDefault extends PhysicalHands {
  ...
  public void layoutHands(BallPathList bpl, double total_time)
      throws JuggleException {
    int i, j;
    double time, delay;
    ...
    for (i = 0; i < hands; i++) {
      ...
      if (bptemp == null) { // hand is inactive for entire pattern
        ...
        while (time < total_time) {
          end = new Coordinate(pos_list[(j + 1) % len][0],
                               pos_list[(j + 1) % len][1]);
          ...
          if ((bptemp2 == null) ||
              (Math.abs(bptemp.getStartTime() - bptemp2.getStartTime()) >
               0.001 / con.tps) ||
...
class PhysicalJugglerDefault extends PhysicalJuggler {
  ...
  double slop_func(double delta, double pos_slop, double neg_slop,
                   double pos_frac, double neg_frac) {
    if (delta > pos_slop)

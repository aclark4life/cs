/* Copyright (c) Alex Clark, 1998. */

/**
 * (c) Copyright 1998, Alex Clark.
 * ALL RIGHTS RESERVED
 * Permission to use, copy, modify, and distribute this software for
 * any purpose and without fee is hereby granted, provided that the above
 * copyright notice appear in all copies and that both the copyright notice
 * and this permission notice appear in supporting documentation, and that
 * the name of Alex Clark not be used in advertising
 ...
 * FITNESS FOR A PARTICULAR PURPOSE.  IN NO EVENT SHALL ALEX CLARK
 * BE LIABLE TO YOU OR ANYONE ELSE FOR ANY DIRECT,
 * SPECIAL, INCIDENTAL, INDIRECT OR CONSEQUENTIAL DAMAGES OF ANY
 * KIND, OR ANY DAMAGES WHATSOEVER, INCLUDING WITHOUT LIMITATION,
 * LOSS OF PROFIT, LOSS OF USE, SAVINGS OR REVENUE, OR THE CLAIMS OF
 * THIRD PARTIES, WHETHER OR NOT ALEX CLARK HAS BEEN
 ...
 * United States.  Contractor/manufacturer is Alex Clark.
 *
 */
...
void get_best_path(t_node n) {
  ...
  float length = get_length(
      n); //	printf ( "<length %f> ", l); //	printf ( "<best %f> ", b);
  ...
  if (length < best_length) {
    ...
    best_length =
        length; // printf ( "<best %f> ", b); 	//printf  ( "<best path");
  }
}
int is_tour(t_node n) {
  if (n->level == d)
...
void get_paths(t_node node) {
  ...
  if (is_promising(node)) {
    ...
    if (is_tour(node)) {
      ...
      //			for (i = 1 ; i <= node->level; i++) { printf
      //("%d ",node->path[i]); } printf (" \n");

...
void gaussj(){
        int i,j,k;
        for (i=j=1;i<=m+1&&j<n;j++){for(k=i;k<=m&&A[k][j]==0;k++); 
		if(k==m+1){solution=FREE;c++;mark[j]=j;markED[c]=j;}else{	
			if(i!=k)swap(i,k);divide(i,j);
			for (k=1;k<=m;k++)if(k!=i){subtract(i,j,k);}i++;}
        } 
	for (i=1;i<=m;i++){
		for (j=1;j<n;j++){
			if (A[i][j]==0)all_zero++;
			if (A[i][j]==0&&j==n-1&&all_zero==n-1) 
				if(A[i][n]!=0)solution=NOSOL;}all_zero=0;
	}
}
void Solution(){
	int i,j,k,var=ASCII; 
	switch(solution){
		case NORM:
			for(i=1;i<=m;i++,var++){printf("%c=%f\n",var,A[i][n]);}
			break;
		case FREE:
			for (i=j=1;i<n;i++,var++){
				if((mark[i])==0){printf("%c=%f",var,A[j][n]);
					for(k=1;k<=c;k++){ printf(" - %f%c",
						A[j][markED[k]],markED[k]+
						ASCII-1);}NL;j++;}
				else{printf("%c=free\n",(mark[i])+(ASCII-1));}}
			break;
		case NOSOL: 
